...
- 修改 `EyeAspectDetector.__init__()` 接受 `baseline_ear` 参数
- 新增 `set_baseline(ear)` 方法动态更新阈值
...
**问题**：眯眼（疲劳表现，持续 >1秒）被误判为眨眼，导致眨眼频率虚高。
...
**算法**：
```
1. EAR 下降阶段：检测到 EAR < 眨眼阈值
2. 计时开始：记录 T_low = EAR 低于阈值的时间
3. 恢复检测：当 EAR 回到睁眼阈值以上
4. 分类判断：
   IF T_low < 400ms → 判定为眨眼，记录眨眼事件
   ELSE → 判定为眯眼，不计入眨眼事件
```

**实现位置**：`detector/eye_aspect.py`
- 修改 `_update_blink_state()` 增加眯眼区分逻辑
- 新增 `_classify_eye_event(T_low)` 方法

**验收标准**：眯眼不被计入眨眼事件

#### T147 阶段三：多信号融合眨眼置信度

**问题**：头部晃动、人员干扰时眨眼检测准确性下降。

**信号融合**：
| 信号 | 处理方式 |
|------|---------|
| 头部姿态 | yaw/pitch 超出阈值时降低眨眼置信度 |
| EAR 变化速率 | 异常快速变化（如10帧内降为0）标记为可疑 |
| face_detected | 短暂丢失(<0.5s)忽略，长时间丢失暂停检测 |
| 眨眼连续性 | 眨眼事件应分散，过密(<0.5s间隔)报警 |
...
**实现位置**：`detector/eye_aspect.py`
- 新增 `_compute_blink_confidence()` 方法
- 修改 `_update_blink_state()` 考虑置信度
