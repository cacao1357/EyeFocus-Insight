...
class CalibrationConfig:
    ...
    head_direction_seconds: float = 2.5   # v4.4: 3.0→2.5 (每个方向2.5s足够转头到位)
    blink_round_seconds: float = 8.0       # v4.4: 15→8 (用户反馈眨眼计数太累)
    blink_rounds_count: int = 1            # v4.4: 2→1 (单轮足够采集眨眼数据)
    ...
    head_direction_min_degrees: float = 15.0    # v4.4: 12→15 (用户实转40-50°, 12°太宽松不准确, 提至15°平衡准确与通过)
