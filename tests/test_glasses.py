...
from analyzer.glasses import (
    ...
    DEFAULT_INNER_CANTHUS_DISTANCE_THRESHOLD,
)
...
def make_landmarks(distance: float = 30.0):
    """创建指定内侧眼角距离的 landmarks
    ...
    center_x = 320
    landmarks[133] = [center_x - distance / 2, 200, 0]  # 左内侧眼角
    landmarks[362] = [center_x + distance / 2, 200, 0]  # 右内侧眼角
    return landmarks
...
class TestGlassesDetector:
    ...
    def test_initial_state(self):
        ...
        assert stats["inner_canthus_dist_thresh"] == DEFAULT_INNER_CANTHUS_DISTANCE_THRESHOLD
    ...
    def test_detect_by_distance_short(self):
        """测试距离检测：distance < 28 → 戴眼镜"""
        detector = GlassesDetector()
        landmarks = make_landmarks(distance=20.0)
        result = detector._detect_by_distance(landmarks)

        assert result is not None
        is_glasses, confidence, distance = result
        assert is_glasses is True
        assert distance < 28.0
        assert 0.0 <= confidence <= 1.0

    def test_detect_by_distance_long(self):
        """测试距离检测：distance > 28 → 不戴眼镜"""
        detector = GlassesDetector()
    ...
    def test_detect_by_distance_boundary(self):
        """测试距离检测：distance ≈ 28 边界情况"""
        detector = GlassesDetector()
        landmarks = make_landmarks(distance=28.0)
        result = detector._detect_by_distance(landmarks)

        assert result is not None
        is_glasses, confidence, distance = result
        # 28.0 不小于 28.0 阈值，所以 is_glasses = False
        assert distance == pytest.approx(28.0, abs=0.001)
        assert is_glasses is False
    ...
    def test_stats(self):
        ...
        assert "inner_canthus_dist_thresh" in stats
...
class TestFactoryFunction:
    ...
    def test_create_glasses_detector(self):
        ...
        assert detector.inner_canthus_dist_thresh == DEFAULT_INNER_CANTHUS_DISTANCE_THRESHOLD
