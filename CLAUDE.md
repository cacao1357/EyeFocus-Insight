...
**修改方案时必须确保每个阶段的计划书包含足够的实现细节：**

| 要求 | 说明 |
|------|------|
| **算法描述** | 每个任务需包含具体算法公式、伪代码或流程图 |
| **参数定义** | 阈值、权重、窗口大小等超参数需列出具体数值及来源 |
| **输入输出** | 函数/方法的输入输出明确，数据类型清晰 |
| **验收标准** | 每个任务需定义可验证的通过条件 |
| **依赖关系** | 任务间依赖需明确说明理由 |
| **异常处理** | 边界情况、错误处理需有说明 |
...
算法：
  ...
  睁眼阈值 = baseline_ear × 0.90

实现：
  1. EyeAspectDetector 新增 baseline_ear 属性
  2. 新增 set_baseline(ear) 方法
  3. 校准完成后 main.py 调用 eye_detector.set_baseline()
