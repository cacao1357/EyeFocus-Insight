...
> **版本**：v4.7 | **制定日期**：2026-05-28 | **修订日期**：2026-06-13
...
> **v4.6 → v4.7**：Phase 2 全部完成 — Insights 实施 + v4.7校准重设计 + 建议引擎升级 + 分心识别 + main.py 自动报告集成 + 408 测试，详见 §16
...
| 11 | 用户辅助校准模块隔离（v4.2 新增） | **独立 `calibration/` 子包 + 模块自有摄像头 + 单接口 `calibration.run()` 连接** | 原 T148 实测全部失效；隔离开发不影响主程序 580 测试；可独立运行 `python -m calibration` 调试 |
...
### v4.4 — GUI 清晰化 + 拖窗口 REC 实施（2026-06-06）

> **变更原因**：v4.3 后实测发现 GUI 信息过载、模式态不清晰、无脸场景无反馈、拖窗口录制无视觉提示。基于 `docs/superpowers/specs/2026-06-05-v4-4-gui-clarity-and-drag-rec.md` 实施，7 commit 完成。
>
> **测试基线**：580/580 通过（`tests/` 382 + `calibration/tests/` 198）

#### 7 项实施明细

| # | Commit | 类型 | 描述 |
|---|--------|------|------|
| 1 | `398a369` | feat(calib) | panel 常驻 ●REC 录制中指示（绿底白字） |
| 2 | `c99a254` | feat(calib) | v4.4 拖窗口检测 + DRAGGING 画面冻结提示 |
| 3 | `b177538` | feat(gui) | v4.4 MODE 状态栏圆点 2.4x + 字体 1.36x + ● 前缀 |
| 4 | `63132db` | feat(gui) | v4.4 focus 圆环 r=70 + 数字 1.5x + 8px 边框颜色按分数 |
| 5 | `dbb0fd4` | feat(gui) | v4.4 fatigue 切档彩色横条（MEDIUM 黄 / HIGH 红闪烁 + 大警告） |
| 6 | `d5b2243` | feat(gui) | v4.4 no-face banner（5s 阈值 + 红底白字横条 + 闪烁） |
| 7 | `691438d` | fix(test) | 6 个 test_input_handler v4.4 回归修复（`__new__` 缺 `_on_waitkey_return`） |

#### 配套文档

| 文档 | 路径 | 状态 |
|------|------|------|
| 设计 spec | `docs/superpowers/specs/2026-06-05-v4-4-gui-clarity-and-drag-rec.md` | ✅ 已批准 + 已实施 |
| 实施 plan | `docs/superpowers/plans/2026-06-05-v4-4-gui-clarity-and-drag-rec.md` | ✅ 7 task, 8 commit 计划 |
| 真机验收 | `docs/REAL_MACHINE_TEST_v4.3.md` (v4.4 共用) | CQS=1.00 |

#### 版本同步

| 文档 | 原版本 | 新版本 | 说明 |
|------|--------|--------|------|
| PROJECT_PLAN.md | v4.3 | v4.4 | 本次更新（已归档 v4.3 → `docs/old_schemes/PROJECT_PLAN_v4.3.md`）|
| PHASE1_PLAN.md | v1.9 | v2.0 | 新增 §2.14 v4.4 维护记录 + 测试数 284 → 580 |
| PHASE2_PLAN.md | v1.2 | v1.3 | 新增 §2.8.3 v4.4 维护记录 + 测试数 284 → 580 |
| MODULE_INTERFACES.md | v1.1 | v1.2 | §2.6 gui/ 重设计 + §2.3 calibration 同步 |
| CLAUDE.md | (gitignored) | §1.4 + §9.1 | 当前版本行 + 测试基线 580 |
...
>
> **注**：v4.3 节点当时测试基线为 284（v4.1 校准未集成时）；v4.4 集成后基线升至 580（`tests/` 382 + `calibration/tests/` 198）。
...
| 项 | 现状 | 是否符合 v4.2 计划 |
|---|------|------------------|
| 284 测试全绿 | ✅ 实测 284/284 通过（v4.3 当时基线；v4.4 升至 580）| ✅ 符合 |
...
### v4.5 — Qt 校准对话框完整实现（2026-06-13）

> **变更原因**：v4.4 Qt 校准对话框存在"点击开始无反应"、固定阈值无法适应个体差异、眨眼验证阈值漂移致 0 次检测、检测器双重初始化精度丢失等严重问题。
>
> **核心改动**：
> - 新增 `gui/calibration_dialog.py`（~770 行），完全替代 OpenCV 校准路径
> - 两阶段数据驱动阈值：睁眼/闭眼实测 EAR → 初始阈值；自然眨眼 nadir P75 → 精细阈值
> - 3 轮眨眼验证闭环：检测数 vs 用户实际数完全一致 ×3 连续方通过；不一致时方向性修正（×1.15/×0.85）+ 硬边界 clamp `[闭眼×0.9, 睁眼×0.85]`
> - 检测器共享：`run_calibration_dialog(fd=fd, ed=ed)` 传入预创建实例，校准/监测共用同一 MediaPipe 实例，避免重复初始化精度下降
> - 按钮位置固定：深色主按钮始终最右（`dots | back_btn | skip_btn | main_btn`）
> - 修复 `qt_window.py` `is_paused()` 死代码：按钮颜色切换逻辑从 return 后不可达代码移至 `set_paused()` 内

**版本同步**：PROJECT_PLAN v4.4 → v4.5 | CLAUDE.md §1.4 同步

### v4.6 — Insights 离线分析子系统 + v4.7 校准对话框重设计（2026-06-13）

> **变更原因**：Phase 2 insights 子系统正式实施（T220-T231）；v4.7 Qt 校准对话框 UI 重设计（70/30 布局 + 白底面板 + 26px 指引文字 + 白底黑字按钮 + 完全手动步骤推进）。

> **核心改动**：
> #### v4.1 Insights 离线分析子系统
> - 新增 `analyzer/insights/` 子包（7 模块）：features, changepoint(PELT), anomaly(IsolationForest), patterns(KMeans+silhouette), temporal(STL/histogram), attribution(t-test+ANOVA), pipeline(orchestration)
> - `run_pipeline(db, session_id)` 统一入口，6 方法独立 try/except 隔离，性能 < 10s
> - 报表集成：`reporter/report_html.py` 新增 `generate_report_with_insights()`，添加 4 个 insights 章节
> - 新增 4 个 Matplotlib 图表（工作模式饼图、异常因子条形图、24h 折线图、关联分析条形图）
> - `storage/db.py` 新增 `insights_cache` 表
> - 单元测试 22 个，覆盖所有子模块
>
> #### v4.7 校准对话框重设计
> - `gui/calibration_dialog.py` 完整重写：70/30 视频/面板比例
> - 白底信息面板 + 浅灰细边框 + 圆角 8px
> - 指引文字 26px / 辅助文字 17px / 倒计时 48px Bold
> - 按钮白底黑字 1.5px 灰边框 44px 高（原深色底白字）
> - 完全手动步骤推进（预告→开始→进行→完成→预告），头部 4 方向独立预告
> - 结果页全宽 2×2 格展示 4 项校准数据

**版本同步**：PROJECT_PLAN v4.5 → v4.6 | CLAUDE.md §1.4 同步

### v4.7 — Phase 2 收官（2026-06-13）

> **变更原因**：Phase 2 全部任务完成收尾。T207 建议引擎升级（attribution findings 驱动个性化建议）、T208-T211 分心识别模块、T202 分心热力条图表、T230 集成测试、T231 文档同步。

> **核心改动**：
> #### T207 建议引擎升级
> - `analyze_with_attributions()`：attribution findings 为主输入，规则引擎降级兜底
> - `attribution_findings_to_insights()`：关联分析发现转 Insight 对象，10 种因子的可执行建议映射
> - `generate_report_with_insights()` 先跑 pipeline → 注入 attribution 建议 → 再加 4+1 章节
>
> #### T208-T211 分心识别模块
> - 新增 `analyzer/distraction.py`：gaze_score + face_detected 分心事件检测
> - 3 类分心事件：短分心（3-15s）/ 中分心（15-60s）/ 长分心（60s+）
> - 3 种模式分类：高频短促 / 间歇中长 / 单次长断
> - 按分钟聚合热力图数据
> - 20 个单元测试
>
> #### T202 分心热力条图表
> - `reporter/charts.py` 新增 `generate_distraction_heatmap()`：彩色横条热力时间轴（绿/黄/红）
> - `reporter/report_html.py` 新增第 5 个 insights 章节：分心分析（含统计 + 热力图 + 模式描述）
>
> #### T230 集成测试
> - 新增 `tests/test_insights_integration.py`（8 测试），覆盖 AC26-AC30
> - AC26：30 sessions × 1h 全 pipeline < 5s（目标 10s）
> - AC27：n < 10 自动降级不报错
> - AC28：异常检测 top 3 中文因子
> - AC30：≥ 1 个统计显著归因发现
>
> #### T231 文档同步
> - PHASE2_PLAN v1.3 → v1.4，全部状态标记 ✅
> - PROJECT_PLAN v4.6 → v4.7，§16 新增 v4.7 条目
> - CLAUDE.md §1.4 同步
>
> #### Bug 修复
> - pipeline.run() 只取 1 个 session → 改为 run_all() 包含全部历史
> - _fetch_all_sessions 查 is_active=1 但会话结束设了 is_active=0 → 去掉 is_active 过滤
> - severe_count 为 None 时除零崩溃 → (row["severe_count"] or 0)
> - anomaly_result 缺 n_sessions 字段 → 加上显示

**版本同步**：PROJECT_PLAN v4.6 → v4.7 | CLAUDE.md §1.4 同步
