...
> **版本**：v2.0 | **制定日期**：2026-05-30 | **最后更新**：2026-06-06
> **阶段目标**：MVP — 核心产品代码实现 + Phase 1.6 Insights Spike 验证 + T148 校准模块标记为废弃
> **负责人**：D1（主开发）、D2（辅助开发）、T1（测试验收）
> **预计工期**：Day 4-13（约 9 天，含 Day 13 的 Phase 1.6 Spike）
> **v1.9 → v2.0**：新增 §2.16 v4.4 维护记录（GUI 清晰化 + 拖窗口 REC）；测试基线 284 → 580

---

## 一、阶段目标

### 1.1 核心交付物（v4.0 MVP — Phase 1 范围）
...
#### 1.1.1 Phase 1 **不含**的模块（v4.1 / v4.2 新增，移交 Phase 2）

| 模块 | 说明 | 移交至 |
|------|------|--------|
| `analyzer/insights/` 子包 | v4.1 离线数据分析（5 个方法：聚类/变点/异常/时序/关联 + pipeline）| Phase 2 §2.6 T220-T231 |
| `calibration/` 子包 | v4.2 用户辅助校准重设计（取代 T148，14 子任务）| Phase 2 §2.0 T-CAL-01~T-CAL-14 |
| `analyzer/user_calibration.py` | T148 老校准（已废弃，607 行，待 T-CAL-14 集成后归档）| Phase 2 §2.0 末尾清理 |
| Phase 1.6 Spike 脚本 | `spike/s11_*.py ~ s15_*.py` + `S-SUM` 报告 | Phase 1.6 门禁（Day 13） |
...
> ⚠️ **【v1.8 标记】T148 已被 v4.2 calibration 模块完整取代**
>
> 2026-06-02 用户实测发现 T148 存在 7 个核心问题（眨眼检测一直 0 / UI 与视频重叠 / 闭眼用户失明 / 头部姿态无指令 / 全程无反馈 / 节奏快无控制权 / 结束无及时退出），属"功能完全实现不了"严重问题。
>
> **后续动作**：在 Phase 2 早期（Day 14-17）由 calibration 模块完整重设计，详见 §2.15 + spec `docs/superpowers/specs/2026-06-02-user-calibration-redesign.md` + PROJECT_PLAN v4.2 §15.4.0。
>
> T148a-T148f 任务保留作为历史记录，但模块代码（`analyzer/user_calibration.py`、`gui/overlay.py` 校准 UI 部分）将在 T-CAL-14 集成成功后归档/删除。

| 任务ID | 描述 | 负责人 | 估时 | 依赖 | 状态 |
|--------|------|--------|------|------|------|
| T148a | analyzer/user_calibration.py 用户校准管理器 | D1 | 2h | T113 | ✅ 已完成（**v1.8 标记废弃，待归档**）|
...
| T148d | gui/overlay.py 新增校准 UI（倒计时/输入/对比）| D2 | 2h | T128 | ✅ 已完成（**v1.8 标记废弃，待删除**）|
| T148e | main.py 集成用户校准流程 | D1 | 1h | T148a, T148d | ✅ 已完成（**v1.8 标记废弃，T-CAL-14 替换**）|
| T148f | 用户校准单元测试 | D2 | 0.5h | T148a | ✅ 已完成（**v1.8 标记废弃**）|
...
| T144 | Phase 1 验收 | T1 | 1.5h | T143 | ✅ 已完成（v1.9 时 284/284；v2.0 升至 580/580 测试通过 + main.py 实测 5+ 分钟无异常） |

### 2.14 Phase 1.6 — Insights Spike 验证（Day 13）【v1.7 新增】

> **背景**：v4.1 引入 `analyzer/insights/` 离线数据分析子包（5 个方法：聚类/变点/异常/时序/关联）。按"路径 A"策略，正式实施前先做 1 天 spike 验证，确认算法在实际数据上的表现并调出推荐参数，再进入 Phase 2 的 T220-T231 实施。
>
> **门禁规则**：S11-S15 全部 PASS 才能进入 T220-T231 正式实施。任一失败需在 S-SUM 中给出降级方案或推迟到 v4.2。

| 任务ID | 描述 | 负责人 | 估时 | 依赖 | 状态 |
|--------|------|--------|------|------|------|
| S11 | `spike/s11_clustering.py` — KMeans + silhouette 自动选 k | D1 | 1.5h | T144 | ⏳ 待执行 |
| S12 | `spike/s12_changepoint.py` — ruptures PELT + 调 penalty | D1 | 1h | T144 | ⏳ 待执行 |
| S13 | `spike/s13_anomaly.py` — IsolationForest + 人造异常 session 测试归因 | D2 | 1h | T144 | ⏳ 待执行 |
| S14 | `spike/s14_temporal.py` — STL 分解，合成 14 天已知 pattern 数据 | D1 | 1h | T144 | ⏳ 待执行 |
| S15 | `spike/s15_attribution.py` — t-test + ANOVA + Cohen's d | D2 | 1h | T144 | ⏳ 待执行 |
| S-SUM | `docs/PHASE1_6_SPIKE_SUMMARY.md` — 5 方法汇总 + 推荐参数表 | D1 | 1h | S11-S15 | ⏳ 待执行 |

#### S11 — 聚类原型验证

**算法**：sklearn KMeans + silhouette_score
**数据来源**：现有 SQLite mock 数据，或合成 30 sessions × 1h（用 v4.1 §6.9.1 中定义的 7 维特征）

**验收标准**：
- 跑通 sklearn pipeline，无运行时错误
- 自动选出的 k ∈ [2, 6]
- silhouette > 0.3
- 模式描述生成 ≥ 1 个可读的中文业务标签（如 "早高峰高效型"）

**输出**：`spike/results/D1/s11_clustering_result.json` + 一段文字结论说明"是否可用于生产"

#### S12 — 变点检测原型验证

**算法**：ruptures.Pelt(model="rbf").predict(pen=penalty)
**数据来源**：现有 session（自然包含转折）或合成"中间有 1 个明显断崖"的 1h 序列

**验收标准**：
- penalty 调到每小时 3-5 个变点
- "中间断崖"测试场景下，检测出的变点时间戳误差 < 30 秒
- 段长 < 60s 的能正确合并

**输出**：标记变点的折线图 + key_moments JSON + 推荐 penalty 系数

#### S13 — 异常检测原型验证

**算法**：sklearn.ensemble.IsolationForest(contamination=0.1)
**数据来源**：20+ 历史 session（正常基线） + 1 个人为构造的异常 session（如 blink_rate 提升 200%）

**验收标准**：
- 人造异常 session 被识别为 anomaly（predict = -1）
- z-score 归因的 top 3 因子中至少 1 个命中人造的偏离特征
- 全 pipeline 耗时 < 500ms

**输出**：anomaly_score + 归因列表，确认"异常归因"对用户有可解释性

#### S14 — 时序分解原型验证

**算法**：statsmodels.tsa.seasonal.STL(period=24, robust=True)
**数据来源**：合成 14 天 × 24 小时 = 336 个数据点，注入已知 pattern（如上午 9-11 点峰值、下午 3-4 点低谷）

**验收标准**：
- 数据 < 7 天时自动 fallback 到 histogram，不报错
- 14 天数据下 STL 收敛
- 恢复出来的 peak hour 与人造一致，误差 ≤ ±1 小时
- 总耗时 < 1 秒

**输出**：daily_pattern[24] 曲线图 + peak_hours/low_hours top 3

#### S15 — 关联分析原型验证

**算法**：scipy.stats.ttest_ind + scipy.stats.f_oneway + Cohen's d
**数据来源**：现有 frames 数据（含 light_level、glasses_mode、hour 等分组维度）

**验收标准**：
- 至少输出 1 个 p<0.05 且 |effect|>0.3 的 finding
- 每个 finding 配有中文 description 和 suggestion
- 单组样本 < 30 时该对比自动跳过（不报错）

**输出**：findings 列表 + 各因素 effect_size 条形图

#### S-SUM — 汇总报告

**输出文档**：`docs/PHASE1_6_SPIKE_SUMMARY.md`

**必含章节**：
1. 各方法 PASS/FAIL 结论
2. 推荐的最终参数表（覆盖 PROJECT_PLAN §6.9 中的默认值）
3. 发现的限制（如哪个方法对数据量敏感、哪个对噪声敏感）
4. 降级触发条件（每个方法）
5. Phase 2 实施风险更新（输入到 PROJECT_PLAN R23-R26）
...
### 2.15 calibration 模块重设计（v4.2 新增）— 跳转到 PHASE2_PLAN

> 🔴 **严重风险任务（R27）**：T148 实测发现 7 个核心问题（"功能完全实现不了"），由 v4.2 calibration 独立模块完整取代。
>
> **不在 Phase 1 内执行**，本节仅作 Phase 1 工作交接 + 指引。

#### 上下文交接（Phase 1 → Phase 2）

| 项 | 内容 |
|------|------|
| 原模块 | `analyzer/user_calibration.py` (607 行) + `gui/overlay.py` 校准 UI 方法（约 200 行）+ `main.py` `CalibrationFlowCallbacks` + `CalibrationCoordinator` 类 |
| 实测发现日期 | 2026-06-02 |
| BUG 数量 | 7 个（详见 spec §0.1）|
| 取代方案 | `calibration/` 独立模块（spec `docs/superpowers/specs/2026-06-02-user-calibration-redesign.md`）|
| Phase 2 任务清单 | T-CAL-01 ~ T-CAL-14（详见 PHASE2_PLAN v1.1 § calibration 模块）|

#### 保守开发策略概要

**实现 → 测试 → 门禁 → 下一步** 严格循环，每个 T-CAL-XX 子任务必须通过：

1. **覆盖率门禁**：单元测试覆盖率达到 spec §6.2 中规定的阈值
2. **真机门禁**（若需）：spike 真机验证通过
3. **集成顺序**：main.py 集成（T-CAL-14）**必须放在最后**，前 13 子任务全过门禁才动 main.py
4. **回退安全**：现有 580 个测试始终绿色，集成失败回滚成本最小

#### Phase 1 收尾注意事项

完成本 Phase 时**不要**做以下事情（留到 Phase 2 calibration 重设计时统一处理）：

- ❌ 不要修改 `analyzer/user_calibration.py`（即使发现 bug 也不修，整个模块即将被替换）
- ❌ 不要修改 `gui/overlay.py` 中的校准 UI 方法
- ❌ 不要修改 `main.py` 中的 `CalibrationFlowCallbacks` / `CalibrationCoordinator` 类
- ✅ 可以修改：`storage/models.py` 中的 `CalibrationResult`、`storage/db.py` 中的 calibration 表（这些在新模块中复用/兼容）

### 2.16 v4.4 维护记录（2026-06-06）【v2.0 新增】

> **背景**：v4.3 后实测发现 GUI 信息过载 + 模式态不清晰 + 拖窗口录制无视觉提示，按 `docs/superpowers/specs/2026-06-05-v4-4-gui-clarity-and-drag-rec.md` 实施。本节仅记录 v4.4 阶段对 Phase 1 已交付模块的**维护性变更**（非新功能）。

| 任务ID | 描述 | 涉及文件 | 状态 |
|--------|------|----------|------|
| V44-1 | 拖窗口检测 + DRAGGING 画面冻结 | `calibration/input_handler.py`, `calibration/ui/panel.py` | ✅ `c99a254` |
| V44-2 | panel 常驻 ●REC 指示（绿底白字）| `calibration/ui/panel.py` | ✅ `398a369` |
| V44-3 | MODE 状态栏圆点 2.4x + 字体 1.36x + ● 前缀 | `gui/overlay.py` | ✅ `b177538` |
| V44-4 | focus 圆环 r=70 + 数字 1.5x + 8px 边框颜色按分数 | `gui/overlay.py` | ✅ `63132db` |
| V44-5 | fatigue 切档彩色横条（MEDIUM 黄 / HIGH 红闪烁）| `gui/overlay.py` | ✅ `dbb0fd4` |
| V44-6 | no-face banner（5s 阈值 + 红底白字 + 闪烁）| `gui/overlay.py`, `main.py` | ✅ `d5b2243` |
| V44-7 | 6 个 test_input_handler v4.4 回归修复 | `calibration/tests/unit/test_input_handler.py` | ✅ `691438d` |

**测试基线变化**：v1.9 时 284 → v2.0 时 580（`tests/` 382 + `calibration/tests/` 198，calibration 子包因 v4.2 模块隔离独立成 test tree）。

**v4.4 不涉及**：检测算法 / 分析模块 / 存储 / 报告生成 — 这些在 v4.1-v4.3 已稳定，本批次不动。
...
| **Phase 1.6 Spike（v1.7）** | 4.5h | 2h | - | 6.5h |
| **合计** | **39.5h** | **29.5h** | **3.5h** | **72.5h** |
...
| 版本 | 日期 | 变更内容 |
|------|------|---------|
| v2.0 | 2026-06-06 | v4.4 同步：新增 §2.16 v4.4 维护记录（7 commit GUI 清晰化 + 1 commit test 回归）；测试基线 284 → 580（`tests/` 382 + `calibration/tests/` 198）|
| v1.9 | 2026-06-02 | v4.3 审计修订：(1) §1.1 标题加"v4.0 MVP — Phase 1 范围"明确版本边界；(2) §1.1 表底新增 §1.1.1 "Phase 1 不含"表，列出 4 个 v4.1/v4.2 模块移交说明（Issue #2 — §1.1 漏声明）；(3) T144 状态 ⏳→✅，并明确 Phase 1 收尾（Issue #3） |
| v1.8 | 2026-06-02 | v4.2 同步：T148 实测失效，标记 T148a-T148f 全部为"已完成但废弃，待 T-CAL-14 集成后归档/删除"；新增 §2.15 calibration 模块重设计 Phase 1 → Phase 2 工作交接 + 保守开发策略概要 + Phase 1 收尾注意事项 |
| v1.7 | 2026-06-02 | v4.1 同步：新增 §2.14 Phase 1.6 Insights Spike（S11-S15+SUM=6 任务，6.5h），含 5 方法验收标准、降级条件、门禁规则；工时汇总 66h→72.5h；工期 Day 4-12→Day 4-13 |
...
| v1.5a | 2026-05-31 | T143 集成测试完成：26 个集成测试，总测试 203 个，通过率 100% |
| v1.4a | 2026-05-31 | T136-T142 单元测试完成：177 测试用例，71% 覆盖率；新增 test_face_mesh.py、test_light.py、test_glasses.py |
...
>
> **【v1.9 状态更新】**T144 已完成。Phase 1 收尾。下一步进入 **Phase 1.6 Spike S11-S15 + S-SUM 门禁**（见 §2.14），通过后再启动 Phase 2（calibration 先行，insights 后行）。
