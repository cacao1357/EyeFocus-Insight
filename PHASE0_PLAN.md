...
| S2-run3 | **D1** | 同上 | 第 3 次基线校准 | 同上 | 3 次 baseline_ear 的 CV < 10%，CQS PASS ≥ 80% |
...
### 2.7 S7：眼镜 + 低光照联合验证（Day 3，全员）

| 任务ID | 负责人 | 描述 | 操作 | 验收标准 |
|--------|--------|------|------|---------|
| S7-a | **戴眼镜成员** | 白天窗边（正面自然光） | 运行 `python spike/fps_benchmark.py` ≥ 2 分钟 | 记录 FPS、置信度均值/最低值 |
| S7-b | **戴眼镜成员** | 晚上台灯（单侧光源） | 同上 ≥ 2 分钟 | 记录 FPS、置信度均值/最低值 |
| S7-c | **戴眼镜成员** | 背光（窗户在身后） | 同上 ≥ 2 分钟 | 记录 FPS、置信度均值/最低值、人脸丢失频率 |
| S7-summary | **全员** | 汇总记录 | 将三种光照数据填入对照表，存入 `spike/s7_lighting_report.txt` | 三种光照 FPS ≥ 20 |
...
|--------|---------|--------|-----------|--------|
| S1 帧率 | 平均 FPS ≥ 25 | | ☐ | D1 |
| S2 基线 | 3 次校准 EAR CV < 10%，CQS PASS ≥ 80% | CV=___%, CQS_PASS=___% | ☐ | D1 |
| S3 姿态 | 正视 yaw_std < 3° 且 pitch_std < 3° | yaw_std=___°, pitch_std=___° | ☐ | D1 |
| S7 光照 | 三种光照 FPS ≥ 20 | | ☐ | 全员 |
